...
with tf.Session(graph=graph,
    ...
    for frame_idx in tqdm(range(0, NUM_TEST_SAMPLE)):
        ...
        # TODO: 下面代码为上面代码的临时替代，正式代码需要恢复
        # 只是模拟降采样代码的赋值过程，没有进行实质性的降采样
        # vertex_coord_list = [cam_rgb_points.xyz]
        # downsampled_xyz = vertex_coord_list[0]
        # vertex_coord_list.append(np.copy(downsampled_xyz))
        ...
        #     np.expand_dims(np.arange(len(vertex_coord_list[0])), axis=1))
        # keypoint_indices_list.append(
        #     np.expand_dims(np.arange(len(vertex_coord_list[1])), axis=1))
        # # keypoint_indices_list.append(
        # #     np.expand_dims(np.arange(len(vertex_coord_list[2])), axis=1))
        # edges_list = []
        # edges_list.append(generate_graph_edges(vertex_coord_list[0], make_undirected=True, add_self_loops=True))
