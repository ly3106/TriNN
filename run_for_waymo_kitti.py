...
from dataset.waymo_kitti_dataset import Points
...
if IS_TEST:
    dataset = WaymoKittiDataset(
        os.path.join(DATASET_DIR, 'testing/image_0/'), # image/testing/image_2
        os.path.join(DATASET_DIR, 'testing/velodyne/'), # velodyne/testing/velodyne/
        os.path.join(DATASET_DIR, 'testing/calib/'), # calib/testing/calib/
        '',
...
if VISUALIZATION_LEVEL == 1:
    ...
    cam_points_in_img_with_rgb = dataset.get_cam_points(0, config['downsample_by_voxel_size'], calib=calib)
    # get_cam_points_in_image_with_rgb(0, calib=calib)
    ...
    # ctr.rotate(0.0, 1178.875, 0) # 3141.0*2/5
    # ctr.scale(-10.0) # Set the zoom (scale) factor to achieve the desired zoom level
    ctr.set_front(np.array([0, 0, 1]))  # 设置z轴正方向指向屏幕观察者，视角的前方向
    ctr.set_lookat(np.array([0, 0, 0]))  # 设置视角的焦点
    ctr.set_up(np.array([0, 1, 0]))  # 设置y轴正方向指向屏幕上，视角的上方向
    # ctr.set_zoom(0.5)  # 设置缩放级别
    vis.run()
...
with tf.Session(graph=graph,
    ...
    for frame_idx in tqdm(range(0, NUM_TEST_SAMPLE)):
        ...
        cam_rgb_points = cam_points = dataset.get_cam_points(frame_idx, config['downsample_by_voxel_size']) # get_cam_points_in_image
        ...
        if box_indices.size != 0:
            ...
            for i in range(len(detection_boxes_3d_corners)):
                ...
                if truncation_rate > 0.4:
                    continue
                x3d, y3d, z3d, l, h, w, yaw = detection_boxes_3d[i] #目标位置
                assert l > 0, str(i)
                score = box_probs[i]
                if USE_BOX_SCORE:
        ...
        if VISUALIZATION_LEVEL > 0:
            # imgname = OUTPUT_DIR + '/img/' + dataset.get_filename(
            #     frame_idx)+'.png'
        ...
        pcdname = OUTPUT_DIR + '/pcd/' + dataset.get_filename(
            frame_idx)+'.png'
        os.makedirs(os.path.dirname(pcdname), exist_ok=True)
        if VISUALIZATION_LEVEL == 1:
            # vis.update_geometry()
            ...
            vis.poll_events()
            vis.update_renderer()
            vis.capture_screen_image(pcdname) # Capture a screenshot
        if VISUALIZATION_LEVEL == 2:
            print("Configure the viewpoint as you want and press [q]")
