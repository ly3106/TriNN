...
class KittiDataset(object):
    ...
    def _verify_file_list(
        ...
        for f in file_list:
            image_file = join(image_dir, f)+'.jpg'
    ...
    def get_calib(self, frame_idx):
        ...
        calib['Tr_velo_to_cam_0'] = calib['Tr_velo_to_cam_0'].reshape(3,4)
        R0_rect = np.eye(4)
        R0_rect[:3, :3] = calib['R0_rect']
        calib['velo_to_rect'] = np.vstack([calib['Tr_velo_to_cam_0'],[0,0,0,1]])
        calib['cam_to_image'] = np.hstack([calib['P0'][:, 0:3], [[0],[0],[0]]])
        calib['rect_to_cam'] = np.hstack([
            calib['R0_rect'],
            np.matmul(
                np.linalg.inv(calib['P0'][:, 0:3]), calib['P0'][:, [3]])])
        ...
        assert np.isclose(calib['velo_to_image'],
            np.matmul(np.matmul(calib['P0'], R0_rect),
    ...
    def get_raw_calib(self, calib_velo_to_cam_path, calib_cam_to_cam_path):
        ...
        calib['Tr_velo_to_cam_0'] = np.vstack(
        ...
        calib['velo_to_cam'] = T2.dot(R0_rect.dot(calib['Tr_velo_to_cam_0']))
    ...
    def get_velo_points(self, frame_idx, xyz_range=None):
        ...
        velo_data = np.fromfile(point_file, dtype=np.float32).reshape(-1, 6)
        velo_points = velo_data[:,:3]
        reflections = np.tanh(velo_data[:,[3]])  # 使用tanh函数标准化intensity字段到[0,1]
    ...
    def get_image(self, frame_idx):
        ...
        image_file = join(self._image_dir, self._file_list[frame_idx])+'.jpg'
