...
PY_CMD="python train_for_waymo_kitti.py \
--dataset_split_file splits/waymo_kitti/include_objects/train_pedestrian_cyclist.txt \
--dataset_root_dir ../dataset/waymo_kitti/ \
configs/waymo_kitti/${CONFIGS_NAME}/train_config.json \
configs/waymo_kitti/${CONFIGS_NAME}/config.json"

#EVAL_PY_CMD="python eval_for_waymo_kitti.py \
#--dataset_root_dir ../dataset/waymo_kitti/ \
#--dataset_split_file ../dataset/waymo_kitti/ImageSets/val.txt \
#configs/waymo_kitti/${CONFIGS_NAME}/eval_config.json"
...
sleep 2s
#gnome-terminal --tab --active -t "record eval.py" -- bash -ic "
#${ACTIVATE_ENV}
#${EVAL_PY_CMD} 2>&1 | tee -a ${TRAIN_DIR}训练中评估打印信息.txt
#exec bash"&
